    ...
    <div class="calendar" v-show="open" ref="calendar">
      <div class="toolbar">
        <div class="toolbar-inner">
          ...
          <div class="calendar-year-picker">
            ...
            <div class="current-year-value" @click="scrollTodayDate">今天</div>
          </div>
        </div>
      </div>
      <div class="calendar-inner">
        ...
        <div class="calendar-month" ref="calendar-touch">
          <div class="calendar-month-wrapper" :style="{'transform': 'translate3d(' + (translateX_parent) + '%, 0, 0)'}">
            <div class="calendar-month-pre" :style="{'transform': 'translate3d(' + (translateX-100) + '%, 0, 0)'}">
              <div class="calendar-month-row" v-for="(row, row_index) in dateObj.pre.allDay_list" :key="row_index">
                <div class="calendar-day" v-for="(col, col_index) in row" :key="col_index" @click="handelDayClick(col,col_index,dateObj.pre.start,dateObj.pre.end)" :class="{'grey':row_index*7+row_index*7+col_index<dateObj.pre.start || row_index*7+col_index>dateObj.pre.end ,'selected':col.selected }">
            ...
            <div class="calendar-month-current" :style="{'transform': 'translate3d(' + translateX + '%, 0, 0)'}">
              <div class="calendar-month-row" v-for="(row, row_index) in dateObj.current.allDay_list" :key="row_index">
                <div class="calendar-day" v-for="(col, col_index) in row" :key="col_index" @click="handelDayClick(col,row_index*7+col_index,dateObj.current.start,dateObj.current.end)" :class="{'grey':row_index*7+col_index<dateObj.current.start || row_index*7+col_index>dateObj.current.end ,'selected':col.selected }">
                  <span :class="{'today':col.isToday,'invalid':col.invalid}">
											{{ col.value }}
										</span>
                </div>
            ...
            <div class="calendar-month-next" :style="{'transform': 'translate3d(' + (translateX+100) + '%, 0, 0)'}">
              <div class="calendar-month-row" v-for="(row, row_index) in dateObj.next.allDay_list" :key="row_index">
                <div class="calendar-day" v-for="(col, col_index) in row" :key="col_index" @click="handelDayClick(col,row_index*7+col_index,dateObj.next.start,dateObj.next.end)" :class="{'grey':row_index*7+col_index<dateObj.next.start || row_index*7+col_index>dateObj.next.end ,'selected':col.selected }">
                  <span :class="{'today':col.isToday,'invalid':col.invalid}">
											{{ col.value }}
										</span>
                </div>
...
export default {
  ...
  watch: {
    open(val) {
      ...
      this._goToCurrentDate(this.currentViewMonth.year, this.currentViewMonth.month)
    },
    value(val) {
      this.open = val;
    }
  ...
  methods: {
    ...
    _goToCurrentDate(year, month) {
      if ((!month || month == this.defaultDate.getMonth() + 1) && (!year || year == this.defaultDate.getFullYear())) {
        return
      }
      if (year && (!month || month == this.defaultDate.getMonth() + 1)) {
        let yearScrollTimes = Math.abs(this.defaultDate.getFullYear() - year)
        for (let i = 0; i < yearScrollTimes; i++) {
          this.handelYearClick(this.defaultDate.getFullYear() < year ? 'pre' : 'next', false) //传true是点击(只滑动一次)
        }
      }
      if (month && (!year || year == this.defaultDate.getFullYear())) {
        let monthScrollTimes = Math.abs(this.defaultDate.getMonth() + 1 - month)
        for (let i = 0; i < monthScrollTimes; i++) {
          this.handelMonthClick(this.defaultDate.getMonth() + 1 < month ? 'pre' : 'next', false) //传true是点击(只滑动一次)
        }
      }
      if ((month || year) && (year !== this.defaultDate.getFullYear() && month !== this.defaultDate.getMonth() + 1)) {
        let yearTimes = Math.abs(this.defaultDate.getFullYear() - year)
        for (let i = 0; i < yearTimes; i++) {
          this.handelYearClick(this.defaultDate.getFullYear() < year ? 'pre' : 'next', false) //传true是点击(只滑动一次)
        }
        let monthTimes = Math.abs(this.defaultDate.getMonth() + 1 - month)
        for (let i = 0; i < monthTimes; i++) {
          this.handelMonthClick(this.defaultDate.getMonth() + 1 < month ? 'pre' : 'next', false) //传true是点击(只滑动一次)
        }
      }
    },
    // _goToCurrentDate() {
    //   if ((!this.currentViewMonth.month || this.currentViewMonth.month == this.defaultDate.getMonth() + 1) && (!_ || _ == this.defaultDate.getFullYear())) {
    //     return
    //   }
    //   if (this.currentViewMonth.year && (!this.currentViewMonth.month || this.currentViewMonth.month == this.defaultDate.getMonth() + 1)) {
    //     let yearScrollTimes = Math.abs(this.defaultDate.getFullYear() - this.currentViewMonth.year)
    //     for (let i = 0; i < yearScrollTimes; i++) {
    //       this.handelYearClick(this.defaultDate.getFullYear() < this.currentViewMonth.year ? 'pre' : 'next', false) //传true是点击(只滑动一次)
    //     }
    //   }
    //   if (this.currentViewMonth.month && (!this.currentViewMonth.year || this.currentViewMonth.year == this.defaultDate.getFullYear())) {
    //     let monthScrollTimes = Math.abs(this.defaultDate.getMonth() + 1 - this.currentViewMonth.month)
    //     for (let i = 0; i < monthScrollTimes; i++) {
    //       this.handelMonthClick(this.defaultDate.getMonth() + 1 < this.currentViewMonth.month ? 'pre' : 'next', false) //传true是点击(只滑动一次)
    //     }
    //   }
    //   if ((this.currentViewMonth.month || this.currentViewMonth.year) && (this.currentViewMonth.year !== this.defaultDate.getFullYear() && this.currentViewMonth.month !== this.defaultDate.getMonth() + 1)) {
    //     let yearTimes = Math.abs(this.defaultDate.getFullYear() - this.currentViewMonth.year)
    //     for (let i = 0; i < yearTimes; i++) {
    //       this.handelYearClick(this.defaultDate.getFullYear() < this.currentViewMonth.year ? 'pre' : 'next', false) //传true是点击(只滑动一次)
    //     }
    //     let monthTimes = Math.abs(this.defaultDate.getMonth() + 1 - this.currentViewMonth.month)
    //     for (let i = 0; i < monthTimes; i++) {
    //       this.handelMonthClick(this.defaultDate.getMonth() + 1 < this.currentViewMonth.month ? 'pre' : 'next', false) //传true是点击(只滑动一次)
    //     }
    //   }
    ...
    //点击"今天"，日历滚动到今天的日期
    scrollTodayDate() {
      let yearSTimes = Math.abs(new Date(Date.now()).getFullYear() - this.currentViewMonth.year)
      for (let i = 0; i < yearSTimes; i++) {
        this.handelYearClick(new Date(Date.now()).getFullYear() < this.currentViewMonth.year ? 'pre' : 'next', false) //传true是点击(只滑动一次)
      }
      let monthSTimes = Math.abs(new Date(Date.now()).getMonth() + 1 - this.currentViewMonth.month)
      for (let i = 0; i < monthSTimes; i++) {
        this.handelMonthClick(new Date(Date.now()).getMonth() + 1 < this.currentViewMonth.month ? 'pre' : 'next', false) //传true是点击(只滑动一次)
      }
    }
  }
}
